...
function getMemBalance(userId: string, symbol: string, snapshot?: Array<{ symbol: string; balance: number; decimals: number }>) {
    ...
    if (balances[userId][symbol] == null) balances[userId][symbol] = 0;
...
async function createSnapshot() {
    try {
        for (const order of open_orders) {
            ...
            const currentBidRice = bidPrices[asset];
            const currentAskPrice = askPrices[asset];
...
async function loadSnapshot() {
    try {
        ...
        console.log("Order IDs loaded:", open_orders.map((order) => `${order.id.slice(0, 8)}...`));
...
async function engine() {
    ...
    while (true) {
        try {
            const res = await client.xread("BLOCK", 0, "STREAMS", ENGINE_STREAM, lastId);
